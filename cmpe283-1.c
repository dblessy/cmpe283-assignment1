 ...
 */
struct capability_info primaryprocessorbased[22] = 
{
 ...
 */
struct capability_info secondaryprocessorbased[28] = 
{
 ...
 */
struct capability_info tertiaryprocessorbased[4] = 
{
        {0, "LOADIWKEY exiting"},
        {1, "Enable HLAT"},
 ...
 */
struct capability_info exitbased[17] = 
{
 ...
 *
 * Checks if true VMX features is available
 *
 * Parameters
 * hi: high 32 bits of cabability MSR describing true vmx controls
 */
bool
detect_true_vmx_features(uint32_t hi)
{
        return (hi & (1 << (55 - 32)));
}

/*
 * detect_vmx_secondar_controls
 *
 * Checks if secondary proc based controls can be set using 'hi'
 *
 * Parameters:
 *   hi: high 32 bits of capability MSR value describing proc based controls
 */
bool
detect_vmx_secondary_controls(uint32_t hi)
{
...
/*
 * detect_vmx_tertiary_controls
 * Check if tertiary proc based controls can be set using 'hi'
 * Parameters:
 *   hi: high 32 bits of capability MSR value describing prcc based controls
 *
 */
bool
detect_vmx_tertiary_controls(uint32_t hi)
{
...
{
        uint32_t lo, hi;
        bool true_vmx_enabled, secondary_controls_enabled, tertiary_controls_enabled;

        /* Check if true VMX cababilities are avilable*/
        rdmsr(IA32_VMX_BASIC, lo, hi);
        true_vmx_enabled = detect_true_vmx_features(hi);
        if (true_vmx_enabled == true) {
                pr_info("True VMX based cababilities are available ");
                /* Pinbased controls */
                ...
                secondary_controls_enabled = detect_vmx_secondary_controls(hi);
                tertiary_controls_enabled = detect_vmx_tertiary_controls(hi);
                pr_info("Primary proc based Controls MSR: 0x%llx\n",
                        (uint64_t)(lo | (uint64_t)hi << 32));
                report_capability(primaryprocessorbased, 22, lo, hi);

                /* Exit controls */
                rdmsr(IA32_VMX_TRUE_EXIT_CTLS, lo, hi);
                pr_info("Exitbased Controls MSR: 0x%llx\n",
                        (uint64_t)(lo | (uint64_t)hi << 32));
                report_capability(exitbased, 17, lo, hi);
                ...
                pr_info("True VMX based capabilities are NOT available");
                /* Pinbased controls */
        ...
        if (secondary_controls_enabled == true) {
                rdmsr(IA32_VMX_PROCBASED_CTLS2, lo, hi);
                pr_info("Secondary proc based controls are enabled\n");
                pr_info("Secondary Proc based Controls MSR: 0x%llx\n",
                        (uint64_t)(lo | (uint64_t)hi << 32));
                report_capability(secondaryprocessorbased, 28, lo, hi);
        } else {
                pr_info("Secondary proc based controls are disabled\n");
        }

        /* Procbased tertiary controls */
        if (tertiary_controls_enabled == true) {
                pr_info("Tertiary proc based controls are enabled\n");
                rdmsr(IA32_VMX_PROCBASED_CTLS3, lo, hi);
                pr_info("Tertiary proc based Controls MSR: 0x%llx\n",
                        (uint64_t)(lo | (uint64_t)hi << 32));
                report_capability(tertiaryprocessorbased, 4, lo, hi);
        } else {
                pr_info("Tertiary proc based controls are disabled\n");
        }
}
